...
import { ClientOptions } from '@nebula-ai/sdk';

const prompt = `Runs JavaScript code to interact with the Nebula API.
...
async function run(client) {
  const collection = await client.collections.create({ name: 'Example collection', description: 'Memory store for my app' });
...
const remoteStainlessHandler = async ({
  ...
  const localClientEnvs = {
    NEBULA_API_KEY: readEnv('NEBULA_API_KEY') ?? client.apiKey ?? undefined,
    NEBULA_BEARER_TOKEN: readEnv('NEBULA_BEARER_TOKEN') ?? client.accessToken ?? undefined,
    NEBULA_BASE_URL: readEnv('NEBULA_BASE_URL') ?? client.baseURL ?? undefined,
  };
...
const localDenoHandler = async ({
  ...
  try {
    const sdkPkgName = '@nebula-ai/sdk';
