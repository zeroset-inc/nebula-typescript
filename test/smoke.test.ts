...
describe("NebulaClient", () => {
  ...
  test("single-collection search derives edge routing header", async () => {
    ...
    expect(calls[0].headers["x-nebula-collection-id"]).toBe(
      "33333333-3333-4333-8333-333333333333"
    );
    expect(calls[1].headers["x-nebula-collection-id"]).toBeUndefined();
  });
  ...
  test("filter-scoped search derives edge routing header", async () => {
    ...
    expect(calls[0].headers["x-nebula-collection-id"]).toBe(collectionId);
    expect(calls[1].headers["x-nebula-collection-id"]).toBeUndefined();
    expect(calls[2].headers["x-nebula-collection-id"]).toBeUndefined();
  });
