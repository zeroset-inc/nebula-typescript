...
describe("NebulaClient", () => {
  ...
  test("workspace-scoped upload uses canonical query routing", async () => {
    ...
    await client.memories.createUpload({
      filename: "image.png",
  ...
  test("keyed memory deletion forwards its key and retries safely", async () => {
    let attempts = 0;
    const { fetchImpl, calls } = makeMockFetch(() => {
      attempts++;
      if (attempts === 1) return jsonResponse(503, { detail: "retry" });
    ...
    const client = new NebulaClient({
      baseUrl: "https://api.example.com",
      fetchImpl,
      retry: { maxRetries: 1, baseMs: 1, maxMs: 1 },
    });

    await client.memories.delete(
      { id: "memory-1", collectionId: "collection-1" },
