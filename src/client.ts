...
export interface ClientOptions {
   ...
   *
   * Defaults to process.env['NEBULA_BASE_URL'].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env['NEBULA_LOG'] or 'warn' if it isn't set.
   */
...
export class Nebula {
  ...
  constructor({
    ...
    this.timeout = options.timeout ?? Nebula.DEFAULT_TIMEOUT /* 1 minute */;
    ...
    this.logLevel =
      ...
      parseLogLevel(readEnv('NEBULA_LOG'), "process.env['NEBULA_LOG']", this) ??
      defaultLogLevel;
    ...
    const customHeadersEnv = readEnv('NEBULA_CUSTOM_HEADERS');
    if (customHeadersEnv) {
  ...
  static NebulaError = Errors.NebulaError;
