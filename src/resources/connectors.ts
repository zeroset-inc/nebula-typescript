...
export class ConnectorsResource {
   ...
   */
  async disconnectHostedM365Tenant(
    params: {
  workspaceId: string;
},
    options?: RequestOptions,
  ): Promise<components["schemas"]["HostedM365TenantDisconnectResponse"]> {
    const response = (await this.core.request<components["schemas"]["WrappedHostedM365TenantDisconnectResponse"]>({
      method: "DELETE",
      ...
      signal: options?.signal,
      timeoutMs: options?.timeoutMs,
    })) as { results: components["schemas"]["HostedM365TenantDisconnectResponse"] };
    return response.results;
  }
