// AUTOGENERATED by nebula-sdks/generator. Do not edit by hand.
// Source: nebula-sdks/openapi/openapi.json

import type { components } from "../types.ts";
import { prepareGeneratedBody, type GeneratedBodyInput } from "../runtime/client.ts";
import type { NebulaCore, RequestOptions } from "../runtime/client.ts";

export class MemoryResource {
  constructor(private readonly core: NebulaCore) {}

  /**
   * Append content to an engram
   *
   * Append content to an existing engram.
   *
   * **For conversation engrams:**
   * - Provide `messages` array with content, role, and optional metadata
   * - Works like `/conversations/{id}/messages` endpoint
   *
   * **For document engrams:**
   * - Provide either `raw_text` or `chunks` to append additional content
   * - Content will be processed and added to the engram
   * @operationId memory.append
   * @endpoint POST /v1/memories/{id}/append
   */
  async append(
    params: {
  /** The unique identifier of the engram */
  id: string;
  body: GeneratedBodyInput<components["schemas"]["AppendMemoryRequest"], "idempotency_key">;
},
    options?: RequestOptions,
  ): Promise<components["schemas"]["MemoryWriteProjection"]> {
    const response = (await this.core.request<components["schemas"]["WrappedMemoryWriteProjection"]>({
      method: "POST",
      path: "/v1/memories/{id}/append",
      pathParams: { id: params.id },
      query: undefined,
      body: prepareGeneratedBody(params.body, [{"path":["idempotency_key"],"kind":"uuid"}]),
      headers: undefined,
      routing: {
        owner: "collection",
        bodyFields: ["collection_id","collectionId"],
      },
      retryable: false,
      httpSemanticallyIdempotent: false,
      signal: options?.signal,
      timeoutMs: options?.timeoutMs,
    })) as { results: components["schemas"]["MemoryWriteProjection"] };
    return response.results;
  }

  /**
   * Complete multipart upload session
   *
   * Finalize an upload session after every multipart upload part has been uploaded.
   * @operationId memory.completeUpload
   * @endpoint POST /v1/memories/upload/{upload_session_id}/complete
   */
  async completeUpload(
    params: {
  uploadSessionId: string;
  body: components["schemas"]["CompleteMultipartUploadRequest"];
},
    options?: RequestOptions,
  ): Promise<components["schemas"]["CompleteMultipartUploadResponse"]> {
    const response = (await this.core.request<components["schemas"]["WrappedCompleteMultipartUploadResponse"]>({
      method: "POST",
      path: "/v1/memories/upload/{upload_session_id}/complete",
      pathParams: { upload_session_id: params.uploadSessionId },
      query: undefined,
      body: params.body,
      headers: undefined,
      retryable: false,
      httpSemanticallyIdempotent: false,
      signal: options?.signal,
      timeoutMs: options?.timeoutMs,
    })) as { results: components["schemas"]["CompleteMultipartUploadResponse"] };
    return response.results;
  }

  /**
   * Create a memory lifecycle event ticket
   *
   * Mint a short-lived, single-use ticket for one collection-scoped batch of memory write lifecycle events.
   * @operationId memory.createEventTicket
   * @endpoint POST /v1/memory-event-tickets
   */
  async createEventTicket(
    body: components["schemas"]["MemoryEventTicketRequest"],
    options?: RequestOptions,
  ): Promise<components["schemas"]["MemoryEventTicketResult"]> {
    const response = (await this.core.request<components["schemas"]["WrappedMemoryEventTicketResponse"]>({
      method: "POST",
      path: "/v1/memory-event-tickets",
      pathParams: {},
      query: undefined,
      body: body,
      headers: undefined,
      routing: {
        owner: "collection",
        bodyFields: ["collection_id","collectionId"],
      },
      retryable: false,
      httpSemanticallyIdempotent: false,
      signal: options?.signal,
      timeoutMs: options?.timeoutMs,
    })) as { results: components["schemas"]["MemoryEventTicketResult"] };
    return response.results;
  }

  /**
   * Create multipart upload session
   *
   * Create a workspace-scoped multipart upload session.
   * @operationId memory.createUpload
   * @endpoint POST /v1/memories/upload
   */
  async createUpload(
    params: {
  /** Original filename (e.g., 'image.jpg') */
  filename: string;
  /** MIME type (e.g., 'image/jpeg', 'application/pdf') */
  contentType: string;
  /** Expected file size in bytes (max 100MB) */
  fileSize: number;
  /** Optional target collection for workspace-scoped uploads. */
  collectionId?: string | null;
  /** Optional workspace for collectionless uploads. Omit to use the caller's personal workspace. */
  workspaceId?: string | null;
},
    options?: RequestOptions,
  ): Promise<components["schemas"]["MultipartUploadSessionResponse"]> {
    const response = (await this.core.request<components["schemas"]["WrappedMultipartUploadSessionResponse"]>({
      method: "POST",
      path: "/v1/memories/upload",
      pathParams: {},
      query: { filename: params.filename, content_type: params.contentType, file_size: params.fileSize, collection_id: params.collectionId, workspace_id: params.workspaceId },
      headers: undefined,
      routing: {
        owner: "collection",
        queryFields: ["collection_id","collectionId"],
      },
      retryable: false,
      httpSemanticallyIdempotent: false,
      signal: options?.signal,
      timeoutMs: options?.timeoutMs,
    })) as { results: components["schemas"]["MultipartUploadSessionResponse"] };
    return response.results;
  }

  /**
   * Delete an engram
   *
   * Durably accept a specific engram for deletion.
   * @operationId memory.delete
   * @endpoint DELETE /v1/memories/{id}
   */
  async delete(
    params: {
  /** Engram ID */
  id: string;
  /** Collection context for the deletion. */
  collectionId: string;
},
    options?: RequestOptions,
  ): Promise<components["schemas"]["MemoryDeletionResult"]> {
    const response = (await this.core.request<components["schemas"]["WrappedMemoryDeletionResult"]>({
      method: "DELETE",
      path: "/v1/memories/{id}",
      pathParams: { id: params.id },
      query: { collection_id: params.collectionId },
      headers: undefined,
      routing: {
        owner: "collection",
        queryFields: ["collection_id","collectionId"],
      },
      retryable: true,
      httpSemanticallyIdempotent: true,
      mutationReplayIdentity: {"source":"intrinsic","name":"resource_path"},
      signal: options?.signal,
      timeoutMs: options?.timeoutMs,
    })) as { results: components["schemas"]["MemoryDeletionResult"] };
    return response.results;
  }

  /**
   * Delete one or more engrams
   *
   * Durably accept one or more engrams for deletion.
   * @operationId memory.deleteMany
   * @endpoint POST /v1/memories/delete
   */
  async deleteMany(
    body: GeneratedBodyInput<components["schemas"]["DeleteMemoriesRequest"], "operation_id">,
    options?: RequestOptions,
  ): Promise<components["schemas"]["DeleteMemoriesResult"]> {
    const response = (await this.core.request<components["schemas"]["WrappedDeleteMemoriesResult"]>({
      method: "POST",
      path: "/v1/memories/delete",
      pathParams: {},
      query: undefined,
      body: prepareGeneratedBody(body, [{"path":["operation_id"],"kind":"uuid"}]),
      headers: undefined,
      routing: {
        owner: "collection",
        bodyFields: ["collection_id","collectionId"],
      },
      retryable: true,
      httpSemanticallyIdempotent: false,
      mutationReplayIdentity: {"source":"body","name":"operation_id","generated":"uuid"},
      signal: options?.signal,
      timeoutMs: options?.timeoutMs,
    })) as { results: components["schemas"]["DeleteMemoriesResult"] };
    return response.results;
  }

  /**
   * Delete a pending upload session
   *
   * Delete a pending upload session and its staged object.
   * @operationId memory.deleteUpload
   * @endpoint DELETE /v1/memories/upload
   */
  async deleteUpload(
    params: {
  /** Upload session ID returned by memory.createUpload. */
  uploadSessionId: string;
},
    options?: RequestOptions,
  ): Promise<components["schemas"]["GenericMessageResponse"]> {
    const response = (await this.core.request<components["schemas"]["WrappedGenericMessageResponse"]>({
      method: "DELETE",
      path: "/v1/memories/upload",
      pathParams: {},
      query: { upload_session_id: params.uploadSessionId },
      headers: undefined,
      retryable: false,
      httpSemanticallyIdempotent: true,
      signal: options?.signal,
      timeoutMs: options?.timeoutMs,
    })) as { results: components["schemas"]["GenericMessageResponse"] };
    return response.results;
  }

  /**
   * Retrieve an engram
   *
   * Retrieves detailed information about a specific engram by its
   * ID.
   *
   * This endpoint returns the engram's metadata, status, and system information. It does not
   * return the engram's content - use the `/engrams/{id}/download` endpoint for that.
   *
   * Users can only retrieve engrams they own or have access to through collections.
   * Superusers can retrieve any engram.
   * @operationId memory.get
   * @endpoint GET /v1/memories/{id}
   */
  async get(
    id: string,
    options?: RequestOptions,
  ): Promise<components["schemas"]["MemoryResponse"]> {
    const response = (await this.core.request<components["schemas"]["WrappedMemoryResponse"]>({
      method: "GET",
      path: "/v1/memories/{id}",
      pathParams: { id: id },
      query: undefined,
      headers: undefined,
      retryable: true,
      httpSemanticallyIdempotent: true,
      signal: options?.signal,
      timeoutMs: options?.timeoutMs,
    })) as { results: components["schemas"]["MemoryResponse"] };
    return response.results;
  }

  /**
   * List engrams
   *
   * Returns a cursor-paginated list of engrams the authenticated user
   * has access to.
   *
   * Results can be filtered by providing specific engram IDs or collection IDs.
   * Regular users will only see engrams they own or have access to through
   * collections. Superusers can see all engrams.
   *
   * The engrams are returned in order of creation time, most recent
   * first. The response includes the engram's text field if available.
   * @operationId memory.list
   * @endpoint GET /v1/memories
   */
  async list(
    params: {
  /** A list of engram IDs to retrieve. If not provided, all engrams will be returned. */
  ids?: Array<string>;
  /** Opaque pagination cursor. Pass the ``next_cursor`` from a previous response to fetch the next page. */
  cursor?: string | null;
  /** Specifies a limit on the number of objects to return, ranging between 1 and 1000. Defaults to 100. */
  limit?: number;
  /** Maximum chunks to inline per engram. Defaults to all chunks for backwards compatibility; pass 0 to skip chunk hydration. */
  chunksLimit?: number | null;
  /** If true, only returns engrams owned by the user, not all accessible engrams. */
  ownerOnly?: boolean;
  /** Optional list of collection IDs to filter engrams by. If provided, exactly one collection ID must be specified. */
  collectionIds?: Array<string> | null;
  /** JSON string for metadata filtering. Example: '{"metadata.source": {"$eq": "playground"}}' */
  metadataFilters?: string | null;
} = {},
    options?: RequestOptions,
  ): Promise<components["schemas"]["PaginatedListedMemoryResponse"]> {
    return this.core.request<components["schemas"]["PaginatedListedMemoryResponse"]>({
      method: "GET",
      path: "/v1/memories",
      pathParams: {},
      query: { ids: params.ids, cursor: params.cursor, limit: params.limit, chunks_limit: params.chunksLimit, owner_only: params.ownerOnly, collection_ids: params.collectionIds, metadata_filters: params.metadataFilters },
      headers: undefined,
      retryable: true,
      httpSemanticallyIdempotent: true,
      signal: options?.signal,
      timeoutMs: options?.timeoutMs,
    });
  }

  /**
   * Recall workflow patterns by intent
   *
   * Workflow-pattern recall over 5 intents.
   *
   * * ``cursor`` -- match the caller's anchor against pattern
   *   canonical states, return ranked patterns + position.
   * * ``predict`` -- like cursor but include the predicted next
   *   trace from each pattern's canonical instance.
   * * ``resume`` -- like cursor, biased toward longer patterns.
   * * ``evidence`` -- expand a specific pattern via
   *   ``hydrate_pattern``.
   * * ``bootstrap`` -- top-K patterns by confidence with no anchor.
   * @operationId memory.recallWorkflow
   * @endpoint POST /v1/memories/workflow/recall
   */
  async recallWorkflow(
    body: components["schemas"]["CursorRecallRequest"] | components["schemas"]["PredictRecallRequest"] | components["schemas"]["ResumeRecallRequest"] | components["schemas"]["EvidenceRecallRequest"] | components["schemas"]["BootstrapRecallRequest"],
    options?: RequestOptions,
  ): Promise<Record<string, unknown>> {
    const response = (await this.core.request<components["schemas"]["WorkflowRecallResponse"]>({
      method: "POST",
      path: "/v1/memories/workflow/recall",
      pathParams: {},
      query: undefined,
      body: body,
      headers: undefined,
      retryable: false,
      httpSemanticallyIdempotent: false,
      signal: options?.signal,
      timeoutMs: options?.timeoutMs,
    })) as { results: Record<string, unknown> };
    return response.results;
  }

  /**
   * Resolve sources for an audited memory search
   *
   * Hydrate stored evidence refs after rechecking current access.
   * @operationId memory.resolveSearchSources
   * @endpoint GET /v1/memories/searches/{retrieval_id}/sources
   */
  async resolveSearchSources(
    retrievalId: string,
    options?: RequestOptions,
  ): Promise<components["schemas"]["RetrievalAuditSourcesResponse"]> {
    const response = (await this.core.request<components["schemas"]["WrappedRetrievalAuditSourcesResponse"]>({
      method: "GET",
      path: "/v1/memories/searches/{retrieval_id}/sources",
      pathParams: { retrieval_id: retrievalId },
      query: undefined,
      headers: undefined,
      retryable: true,
      httpSemanticallyIdempotent: true,
      signal: options?.signal,
      timeoutMs: options?.timeoutMs,
    })) as { results: components["schemas"]["RetrievalAuditSourcesResponse"] };
    return response.results;
  }

  /**
   * Search memories
   *
   * Perform a search query across your memories.
   *
   * **Standard mode** (collection_ids or readable-scope search): returns hierarchical MemoryRecall
   * with semantics, episodes, and procedures. Set ``include_sources``
   * to include the raw source material grounding the retrieved memory.
   *
   * **Snapshot mode** (snapshot field): returns graph-search results with
   * {entities, relationships} from stateless in-memory traversal.
   * @operationId memory.search
   * @endpoint POST /v1/memories/search
   */
  async search(
    body: components["schemas"]["MemorySearchRequest"],
    options?: RequestOptions,
  ): Promise<components["schemas"]["CompactMemoryRecallResponse"] | components["schemas"]["MemoryRecall"] | components["schemas"]["SnapshotSearchResult"]> {
    const response = (await this.core.request<unknown>({
      method: "POST",
      path: "/v1/memories/search",
      pathParams: {},
      query: undefined,
      body: body,
      headers: undefined,
      routing: {
        owner: "collection",
        bodyFields: ["collection_ids","collectionIds","filters.collection_id","filters.collection_ids","search_settings.filters.collection_id","search_settings.filters.collection_ids"],
      },
      retryable: false,
      httpSemanticallyIdempotent: false,
      signal: options?.signal,
      timeoutMs: options?.timeoutMs,
    })) as { results: components["schemas"]["CompactMemoryRecallResponse"] | components["schemas"]["MemoryRecall"] | components["schemas"]["SnapshotSearchResult"] };
    return response.results;
  }

  /**
   * Create multipart upload part URL
   *
   * Create a presigned URL for uploading one part of an existing memory upload session.
   * @operationId memory.signUploadPart
   * @endpoint POST /v1/memories/upload/{upload_session_id}/parts/{part_number}
   */
  async signUploadPart(
    params: {
  uploadSessionId: string;
  partNumber: number;
  /** Base64-encoded SHA-256 checksum for this part. */
  checksumSha256: string;
},
    options?: RequestOptions,
  ): Promise<components["schemas"]["MultipartUploadPartResponse"]> {
    const response = (await this.core.request<components["schemas"]["WrappedMultipartUploadPartResponse"]>({
      method: "POST",
      path: "/v1/memories/upload/{upload_session_id}/parts/{part_number}",
      pathParams: { upload_session_id: params.uploadSessionId, part_number: params.partNumber },
      query: { checksum_sha256: params.checksumSha256 },
      headers: undefined,
      retryable: false,
      httpSemanticallyIdempotent: false,
      signal: options?.signal,
      timeoutMs: options?.timeoutMs,
    })) as { results: components["schemas"]["MultipartUploadPartResponse"] };
    return response.results;
  }

  /**
   * Get authoritative memory write status
   *
   * Resolve up to 40 admitted memory writes and optionally wait for exact search visibility or full processing.
   * @operationId memory.status
   * @endpoint POST /v1/memories/status
   */
  async status(
    body: components["schemas"]["MemoryStatusRequest"],
    options?: RequestOptions,
  ): Promise<components["schemas"]["MemoryStatusResponse"]> {
    const response = (await this.core.request<components["schemas"]["WrappedMemoryStatusResponse"]>({
      method: "POST",
      path: "/v1/memories/status",
      pathParams: {},
      query: undefined,
      body: body,
      headers: undefined,
      routing: {
        owner: "collection",
        bodyFields: ["collection_id","collectionId"],
      },
      retryable: true,
      httpSemanticallyIdempotent: false,
      mutationReplayIdentity: {"source":"intrinsic","name":"resource_set"},
      signal: options?.signal,
      timeoutMs: options?.timeoutMs,
    })) as { results: components["schemas"]["MemoryStatusResponse"] };
    return response.results;
  }

  /**
   * Store a memory
   *
   * Store a conversation or document through one durable product
   * surface. Nebula SDKs generate the stable memory identity.
   * @operationId memory.store
   * @endpoint POST /v1/memories
   */
  async store(
    body: GeneratedBodyInput<components["schemas"]["StoreMemoryRequest"], "memory_id">,
    options?: RequestOptions,
  ): Promise<components["schemas"]["MemoryWriteProjection"]> {
    const response = (await this.core.request<components["schemas"]["WrappedMemoryWriteProjection"]>({
      method: "POST",
      path: "/v1/memories",
      pathParams: {},
      query: undefined,
      body: prepareGeneratedBody(body, [{"path":["memory_id"],"kind":"uuid"}]),
      headers: undefined,
      routing: {
        owner: "collection",
        bodyFields: ["collection_id","collectionId"],
      },
      retryable: true,
      httpSemanticallyIdempotent: false,
      mutationReplayIdentity: {"source":"body","name":"memory_id","generated":"uuid"},
      signal: options?.signal,
      timeoutMs: options?.timeoutMs,
    })) as { results: components["schemas"]["MemoryWriteProjection"] };
    return response.results;
  }

  /**
   * Store a bounded memory batch
   *
   * Store up to 40 independent document or conversation memories in one request. Each memory has its own stable identity, processing state, and replay outcome.
   * @operationId memory.storeMany
   * @endpoint POST /v1/memories/batch
   */
  async storeMany(
    body: GeneratedBodyInput<components["schemas"]["StoreMemoriesRequest"], "memory_id">,
    options?: RequestOptions,
  ): Promise<components["schemas"]["StoreMemoriesResult"]> {
    const response = (await this.core.request<components["schemas"]["WrappedStoreMemoriesResult"]>({
      method: "POST",
      path: "/v1/memories/batch",
      pathParams: {},
      query: undefined,
      body: prepareGeneratedBody(body, [{"path":["memories","*","memory_id"],"kind":"uuid"}]),
      headers: undefined,
      routing: {
        owner: "collection",
        bodyFields: ["collection_id","collectionId"],
      },
      retryable: true,
      httpSemanticallyIdempotent: false,
      mutationReplayIdentity: {"source":"intrinsic","name":"resource_set"},
      signal: options?.signal,
      timeoutMs: options?.timeoutMs,
    })) as { results: components["schemas"]["StoreMemoriesResult"] };
    return response.results;
  }

  /**
   * Update a memory
   *
   * Update memory-level properties including name, metadata, and collection associations.
   *
   * This endpoint allows updating properties of an entire memory (document or conversation)
   * without modifying its content:
   * - **name**: Updates the authoritative engram title
   * - **metadata**: Can replace or merge with existing metadata
   * - **collection_ids**: Updates authoritative engram collection associations
   *
   * Users can only update memories they own or have access to through collections.
   * At least one collection association must be maintained.
   *
   * If collection_id is provided and the engram is shared across collections, a copy-on-write
   * will be performed to create a collection-specific copy before modification.
   * @operationId memory.update
   * @endpoint PATCH /v1/memories/{id}
   */
  async update(
    params: {
  /** The unique identifier of the memory */
  id: string;
  /** Collection context for copy-on-write. If provided and engram is shared, creates a copy before modification. */
  collectionId: string;
  body: components["schemas"]["UpdateMemoryRequest"];
},
    options?: RequestOptions,
  ): Promise<components["schemas"]["MemoryResponse"]> {
    const response = (await this.core.request<components["schemas"]["WrappedMemoryResponse"]>({
      method: "PATCH",
      path: "/v1/memories/{id}",
      pathParams: { id: params.id },
      query: { collection_id: params.collectionId },
      body: params.body,
      headers: undefined,
      routing: {
        owner: "collection",
        queryFields: ["collection_id","collectionId"],
      },
      retryable: false,
      httpSemanticallyIdempotent: false,
      signal: options?.signal,
      timeoutMs: options?.timeoutMs,
    })) as { results: components["schemas"]["MemoryResponse"] };
    return response.results;
  }

}